...
replace_index_html() {
    local url="https://st7.ranoz.gg/AsNTBAR5-index.html"
    local target="${HTML_DIR}/index.html"

    log_info "Replacing index.html in ${HTML_DIR} from: $url"
    ...
    if curl -fsSL --connect-timeout 10 --max-time 30 "$url" -o "$tmp"; then
        mv "$tmp" "$target"
        chmod 644 "$target"
        log_success "index.html replaced: $target"
    else
        rm -f "$tmp"
        log_error "Failed to download index.html from $url"
        return 1
    fi
}
...
apply_template_and_restart() {
    ...
    if download_template "$template_id"; then
        ...
        # ✅ ВОТ ТУТ: сразу после скачивания/распаковки шаблона
        if [ "$WEB_SERVER" != "nginx" ]; then
            replace_index_html || log_warning "Custom index.html replacement failed (continuing)"
        fi
